...
class nnUNetLogger(object):
    ...
    def load_checkpoint(self, checkpoint: dict):
        ...
        for key in ('mean_fg_dice', 'ema_fg_dice', 'dice_per_class_or_region',
            ...
            self.my_fantastic_logging.setdefault(key, list())



class nnUNetLoggerCBS(object):
    """
    This class is really trivial. Don't expect cool functionality here. This is my makeshift solution to problems
    arising from out-of-sync epoch numbers and numbers of logged loss values. It also simplifies the trainer class a
    little
    ...
    def __init__(self, verbose: bool = False):
        self.my_fantastic_logging = {
            ...
            'epoch_start_timestamps': list(),
            'epoch_end_timestamps': list(), 
            'consistency_weight': list(), 
            'weighted_loss_ml': list(),
            'loss_ml': list(),
            'loss_cls': list(),
            'weak_penalty_weight': list(),
        }
        self.verbose = verbose
        # shut up, this logging is great
    ...
    def plot_progress_png(self, output_folder):
        # Use only the standard keys for epoch calculation (extra SSL keys may lag behind)
        standard_keys = ['mean_fg_dice', 'ema_fg_dice', 'dice_per_class_or_region',
                         'train_losses', 'val_losses', 'lrs',
                         'epoch_start_timestamps', 'epoch_end_timestamps']
        epoch = min([len(self.my_fantastic_logging[k]) for k in standard_keys]) - 1
        sns.set(font_scale=2.5)
        fig, ax_all = plt.subplots(3, 1, figsize=(30, 54))
        # regular progress.png as we are used to from previous nnU-Net versions
        ax = ax_all[0]
    ...
    def load_checkpoint(self, checkpoint: dict):
        ...
        for key in ('mean_fg_dice', 'ema_fg_dice', 'dice_per_class_or_region',
                'train_losses', 'val_losses', 'lrs',
                'epoch_start_timestamps', 'epoch_end_timestamps',
                'consistency_weight', 'weighted_loss_ml', 'loss_ml',
                'loss_cls', 'weak_penalty_weight'):
            self.my_fantastic_logging.setdefault(key, list())