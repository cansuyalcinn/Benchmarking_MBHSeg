...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
                 device: torch.device = torch.device('cuda'), percentage_labeled_data: float = 1.0, seed_name: str = '0', seed_balance = '42'):
        ...
        self.output_folder_base = join(
            ...
            f"{self.__class__.__name__}__{self.plans_manager.plans_name}__{configuration}__perc{self.percentage_labeled_data}_seed{self.seed_name}_seedbalance{self.seed_balance}"
    ...
    def do_split(self):
        ...
        self.print_to_log_file(f"Using {len(labeled_cases)} labeled training cases out of {len(tr_keys)} total training cases. {len(val_keys)} validation cases")
