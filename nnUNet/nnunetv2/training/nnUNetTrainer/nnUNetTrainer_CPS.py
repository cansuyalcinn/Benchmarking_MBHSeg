...
# CPS model.
# contains two nnunets that were initialized with the same encoder and decoder architecture, but different weights.
# we are using the first network for loading - inference and validation step (by default nnunet)
...
class nnUNetTrainerCPS(nnUNetTrainerNoDeepSupervision):
    """
    ...
    def __init__(
        ...
        # Toggle mixed-precision (autocast) for CPS training.
    ...
    def configure_optimizers2(self):
        optimizer = torch.optim.SGD(self.network2.parameters(), 
                                    self.initial_lr, 
                                    weight_decay=self.weight_decay,
                                    momentum=0.99, 
                                    nesterov=True)
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.optimizer2, self.lr_scheduler2 = self.configure_optimizers2()

            # if ddp, wrap in DDP wrapper
            if self.is_ddp:
                self.network = torch.nn.SyncBatchNorm.convert_sync_batchnorm(self.network)
                self.network = DDP(self.network, device_ids=[self.local_rank])
                self.network2 = torch.nn.SyncBatchNorm.convert_sync_batchnorm(self.network2)
                self.network2 = DDP(self.network2, device_ids=[self.local_rank])

            self.loss = self._build_loss()
            self.loss2 = self._build_loss()
    ...
    def do_split(self):
        ...
        if self.fold == "all":
            ...
            if self.fold < len(splits):
                ...
                # ONLY return early if 100% labeled
    ...
    @staticmethod
    # We create nnunet but without deep supervision. 
    ...
    def _cps_loss(self, output, pseudo_label):
        """
        Cross-Pseudo Supervision (CPS) loss function from paper: "Semi-Supervised Semantic Segmentation with Cross-Pseudo Supervision"
        
        This function computes the consistency loss between the logits from one network and the 
        pseudo-labels generated from the other network. It's core to the CPS training strategy 
        where two networks learn from each other's predictions on unlabeled data.
        
        Args:
            output: Logits/predictions from one network. Shape: (batch_size, num_classes, spatial_dims...)
                   For segmentation, typically (B, C, H, W, D) for 3D or (B, C, H, W) for 2D
            pseudo_label: Pseudo-labels generated from the other network. These are the argmax predictions
                         from the other network. Shape: (batch_size, 1, spatial_dims...) for multi-class
                         or (batch_size, spatial_dims...) after squeezing
        
        Returns:
            loss: Scalar tensor representing the mean consistency loss. This loss penalizes the current 
                  network when its predictions disagree with the pseudo-labels from the other network.
        
        Workflow:
        1. Squeeze the pseudo-label channel dimension if present (for multi-class segmentation)
        2. Use CrossEntropyLoss with no reduction (reduction='none') to compute per-element losses
        3. The loss is computed element-wise across the spatial dimensions
        4. Return the mean loss across all elements
        """
        # Squeeze pseudo_label to remove the channel dimension for CrossEntropyLoss
        # pseudo_label shape: (B, 1, H, W, D) -> (B, H, W, D)
        pseudo_label = pseudo_label.squeeze(1)
        
        # Compute the consistency loss using CrossEntropyLoss
        # output: (B, C, H, W, D), pseudo_label: (B, H, W, D)
        # The loss is computed at each spatial location and averaged
        cps_loss = self.consistency_criterion_ml(output, pseudo_label)
        
        # Return the mean loss across all spatial locations and batch
        return cps_loss.mean()
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        # Handle target list (Deep Supervision) or single tensor
        if isinstance(target, list):
            target = [i.to(self.device, non_blocking=True) for i in target]
            # Slice the targets to only keep the labeled part for supervised loss
        ...
        # 2. Split Data for Forward Pass (Prevents BN Contamination)
        data_labeled = data[:actual_labeled_bs]
        data_unlabeled = data[actual_labeled_bs:]

        # Autocast for mixed-precision training (matches baseline nnUNetTrainer behavior)
        with autocast(self.device.type, enabled=True) if (self.device.type == 'cuda' and self.use_autocast) else dummy_context():
            # --- Forward both networks on full batch (labeled + unlabeled concatenated) ---
            # 2 forward passes instead of 4: more GPU-efficient and gives BN better statistics
            data_all = torch.cat([data_labeled, data_unlabeled], dim=0)
            out1_all = self.network(data_all)
            out2_all = self.network2(data_all)
            out1_labeled, out1_unlabeled = out1_all[:actual_labeled_bs], out1_all[actual_labeled_bs:]
            out2_labeled, out2_unlabeled = out2_all[:actual_labeled_bs], out2_all[actual_labeled_bs:]

            # 3. Supervised Loss (Only on Labeled Part)
            l_sup1 = self.loss(out1_labeled, target_labeled)
            l_sup2 = self.loss(out2_labeled, target_labeled)

            # 4. CPS Logic
            # Reconstruct full outputs for CPS calculation if needed, or just use unlabeled parts
            # CPS loss is usually calculated on the UNLABELED data only (or both). 
            # Standard CPS paper uses both, but let's stick to your implementation which uses Unlabeled.
            
            # Generate Pseudo Labels from Unlabeled data
            if self.label_manager.has_regions:
                pseudo1 = (torch.sigmoid(out1_unlabeled) > 0.5).long()
                pseudo2 = (torch.sigmoid(out2_unlabeled) > 0.5).long()
            else:
                pseudo1 = out1_unlabeled.argmax(1, keepdim=True)
                pseudo2 = out2_unlabeled.argmax(1, keepdim=True)

            # Compute consistency loss
            l_cps_1 = self._cps_loss(out1_unlabeled, pseudo2)
            l_cps_2 = self._cps_loss(out2_unlabeled, pseudo1)
            ...
            # Following the original implementation of the losses. From Chen Xiaokang et al., “Semi-Supervised Semantic Segmentation with Cross Pseudo Supervision” (CVPR 2021)
            # L=Lsup​+λLcps
            # Different from the original paper we introduce the ramp-up function for the consistency weight. 

            model1_loss = l_sup1 + ssl_weight * l_cps_1
            model2_loss = l_sup2 + ssl_weight * l_cps_2

            l = model1_loss + model2_loss
        ...
        return {
            ...
            'loss_ml': (l_cps_1 + l_cps_2).detach().cpu().numpy(),
            'weighted_loss_ml': (ssl_weight * (l_cps_1 + l_cps_2)).detach().cpu().numpy(),
        }
    ...
    def on_train_epoch_end(self, train_outputs: list):
        ...
        self.logger.log('loss_cls', 0.0, self.current_epoch)  # no classification loss in CPS
