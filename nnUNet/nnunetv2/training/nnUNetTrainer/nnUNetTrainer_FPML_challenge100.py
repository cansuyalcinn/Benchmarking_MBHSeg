...
class nnUNetTrainerFPMLchallenge_100(nnUNetTrainerNoDeepSupervision):
    """
    ...
    def do_split(self):
        ...
        if self.fold == "all":
            ...
            # we add + 100% of unlabeled cases
            tr_unlabeled_keys = tr_unlabeled_keys[:121]

        self.print_to_log_file(f"Using {len(labeled_cases)} labeled training cases out of {len(tr_keys)} total training cases. {len(tr_unlabeled_keys)} unlabeled training cases. {len(val_keys)} validation cases")
