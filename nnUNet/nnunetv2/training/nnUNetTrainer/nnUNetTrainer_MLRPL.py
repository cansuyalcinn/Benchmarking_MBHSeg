...
class nnUNetTrainerMLRPL(nnUNetTrainerNoDeepSupervision):
    """
    ...
    def calculate_ML(self, y_all, outfeats, outputs_seg, i, j, num_classes, consistency_criterion):
        uncertainty_o1 = -1.0 * torch.sum(y_all[i] * torch.log(y_all[i] + 1e-6), dim=1) #ca
        uncertainty_o2 = -1.0 * torch.sum(y_all[j] * torch.log(y_all[j] + 1e-6), dim=1) #cb
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if (self.device.type == 'cuda' and self.use_autocast) else dummy_context():
            ...
            for i in range(num_outputs):
                for j in range(num_outputs):
                    if i != j:
                        loss_ml += self.calculate_ML(y_all,
                                                    outfeats,
                                                    outputs_seg,
                                                    i,
                                                    j,
                                                    self.label_manager.num_segmentation_heads,
                                                    self.consistency_criterion_ml)

            cw_ml = self.get_current_consistency_weight(start_epoch=0, end_epoch=1000, max_weight=0.1)
            weighted_loss_ml_fp = cw_ml * loss_ml.detach() if torch.is_tensor(loss_ml) else cw_ml * float(loss_ml)
            l = (0.5 * loss_seg) + cw_ml * loss_ml
        ...
        return {
            ...
            'loss_ml': loss_ml.detach().cpu().numpy() if torch.is_tensor(loss_ml) else float(loss_ml),
