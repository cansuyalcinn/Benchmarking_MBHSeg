...
def get_trainer_from_args(dataset_name_or_id: Union[int, str],
    ...
    nnunet_trainer = nnunet_trainer(plans=plans, configuration=configuration, fold=fold,
                                ...
                                percentage_labeled_data=percentage, seed_name=seed_name, seed_balance=seed_balance)
...
def run_training(dataset_name_or_id: Union[str, int],
                 ...
                 seed_balance : str = '42' # seed to select which balanced split.
                 ):
    ...
    if num_gpus > 1:
        ...
        nnunet_trainer = get_trainer_from_args(dataset_name_or_id, configuration, fold, trainer_class_name,
                                               plans_identifier, device=device, percentage=percentage, seed_name=seed_name, seed_balance=seed_balance)

        if disable_checkpointing:
            nnunet_trainer.disable_checkpointing = disable_checkpointing

        assert not (continue_training and (only_run_validation or val_with_best)), f'Cannot set --c and --val/--val_best flag at the same time. Dummy.'

        maybe_load_checkpoint(nnunet_trainer, continue_training, only_run_validation or val_with_best, pretrained_weights)

        if torch.cuda.is_available():
            cudnn.deterministic = False
            cudnn.benchmark = True

        if not (only_run_validation or val_with_best):
            nnunet_trainer.run_training()

        # Always load checkpoint_best for validation (unless it doesn't exist)
        # We changed tha last validation to use chcekpoint_best.
        best_ckpt = join(nnunet_trainer.output_folder, 'checkpoint_best.pth')
        if isfile(best_ckpt):
            nnunet_trainer.load_checkpoint(best_ckpt)
...
def run_training_entry():
    ...
    parser.add_argument('--seed_balance', '-seedb', type=str, default='42', required=False) # seed to select which balanced split. 
    ...
    run_training(args.dataset_name_or_id, args.configuration, args.fold, args.tr, args.p, args.pretrained_weights,
                 ...
                 device=device, percentage=args.percentage_labeled_data, seed_name=args.seed_name, seed_balance=args.seed_balance)
