...
class nnUNetPredictor(object):
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        print("trainer class is:", trainer_class)

        # Inspect the signature of build_network_architecture to decide which
        # calling convention to use.  The CPS (and similar) trainers override
        # the method with (plans_manager, dataset_json, configuration_manager,
        # num_input_channels, enable_deep_supervision) whereas the default
        # nnUNetTrainer uses (arch_class_name, arch_init_kwargs, ...).
        import inspect
        sig = inspect.signature(trainer_class.build_network_architecture)
        param_names = list(sig.parameters.keys())

        if 'plans_manager' in param_names:
            # CPS-style signature
            ...
            # Default nnUNetTrainer signature
            network = trainer_class.build_network_architecture(
        ...
        # Enable inference mode for dual-decoder models (e.g., DCNET, CPS) so they return tensor instead of dict
        if hasattr(network, 'inference_mode'):
            network.inference_mode = True
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor) -> torch.Tensor:
        ...
        def get_primary_seg_output(pred):
            # Handle models returning dict with 'seg1' or 'output1' (custom dual-decoder e.g. CPS, DCNET)
    ...
    def predict_from_files_sequential2(self,
                           list_of_lists_or_source_folder: Union[str, List[List[str]]],
                           output_folder_or_list_of_truncated_output_files: Union[str, None, List[str]],
                           save_probabilities: bool = False,
                           overwrite: bool = True,
        ...
        for li, of, sps in zip(list_of_lists_or_source_folder, output_filename_truncated, seg_from_prev_stage_files):
            ...
            print(f'perform_everything_on_device: {self.perform_everything_on_device}')
            prediction = self.predict_logits_from_preprocessed_data(torch.from_numpy(data)).cpu()
            # Instead of saving or postprocessing, just collect the raw logits
            ret.append(prediction.numpy())
